...
class PendapatanController extends Controller
{
    ...
    public function process(Request $request)
    {
        $request->validate([
            ...
            'schema_id'   => 'nullable',
            'header_hash' => 'nullable|string', // fallback kalau schema_id tidak dikirim
        ]);
        ...
        try {
            ...
            if (!$schemaId) {
                throw new \Exception('Schema ID tidak ditemukan. Pastikan store sudah mengembalikan schema_id.');
            }

            // 🔹 Ambil semua row yang sudah ada untuk schema + seller
            ...
            foreach ($spreadsheet->getWorksheetIterator() as $sheet) {
                ...
                for ($row = $dataStartRow; $row <= $highestRow; $row++) {
                    ...
                    foreach ($request->columns as $col => $header) {
                        $raw = $sheet->getCell($col . $row)->getValue();
                        $item[$header] = $request->has("convert.$col")
                    ...
                    // Tambahkan ke buffer & existingRows
                    $existingRows[] = $item;
                    $buffer[] = $item;
                    $totalRows++;
